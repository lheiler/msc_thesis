...
# Age regression dataset: normal EEGs across age range
if "AgeAtVisit_x" in df.columns:
    age_filtered = normal_df[normal_df["AgeAtVisit_x"].between(18, 80)]
    if len(age_filtered) < 500:
        raise ValueError(f"Not enough age-regression EEGs. Found {len(age_filtered)}, need at least 500.")
    age_regression_sample = age_filtered.sample(n=500, random_state=42)
else:
    age_regression_sample = pd.DataFrame()

filtered = age_regression_sample.copy()
...
def download_session(row):
    ...
    local_path = os.path.join(desired_dir, "bids_age_500", subject, session)
