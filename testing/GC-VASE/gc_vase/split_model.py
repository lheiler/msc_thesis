...
try:
    ...
    def global_mean_pool(x, batch):
        # x: (N, C); batch: (N,) indices. Compute per-graph mean if possible
        if batch is None:
            return x.mean(dim=0, keepdim=True)
        unique = torch.unique(batch)
        outs = []
        for b in unique:
            outs.append(x[batch == b].mean(dim=0, keepdim=True))
...
class StridedConvolutionalEncoder(nn.Module):
    ...
    def forward(self, x, edge_index=None, batch=None, cond=None):
        ...
        if HAS_PYG and edge_index is not None and batch is not None:
    ...
    def freeze_gnn_layers(self):
        for p in self.gnn_layers.parameters():
