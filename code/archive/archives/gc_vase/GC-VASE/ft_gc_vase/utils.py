...
class DelegatedLoader(IterableDataset):
    def __init__(self, loader, property=None, batch_size=None, length=None):
        ...
        self._length = length if length is not None else len(loader.data)
...
class CustomLoader:
    def __init__(self, data_dict, split='train', exclude_subjects=[]):
        self.split = split
        ...
        all_test_splits = [5, 14, 15, 20, 22, 23, 26, 29]
        #fine_tune_splits = []
        train_splits = [1, 2, 3, 6, 8, 9, 10, 11, 12, 13, 16, 17, 19, 21, 24, 25, 28, 31, 32, 34, 35, 36, 37, 38, 39, 40, 18, 30] 
        
        np.random.shuffle(all_test_splits)
        split_index = int(0.8 * len(all_test_splits)) #0.7
        fine_tune_splits = all_test_splits[:split_index]
        test_splits = all_test_splits[split_index:]

        if split == 'dev':
            self.unique_subjects = dev_splits
        elif split == 'fine_tune':
            self.unique_subjects = fine_tune_splits
        elif split == 'test':
            self.unique_subjects = test_splits
        elif split == 'train':
            self.unique_subjects = [s for s in train_splits if s not in exclude_subjects]
        elif split == 'N170':
