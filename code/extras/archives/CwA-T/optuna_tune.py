...
def build_search_space(cfg: dict, trial: optuna.trial.Trial) -> dict:
    ...
    # Cosine schedule minimum LR (must be < init_lr)
    cfg["scheduler"]["lr_min"] = trial.suggest_float("lr_min", 5e-6, cfg["optimizer"]["init_lr"] * 0.9, log=True)

    # Model architecture
    cfg["model"]["d_model"] = trial.suggest_categorical("d_model", [128, 192, 256])
    cfg["model"]["n_head"] = trial.suggest_categorical("n_head", [2, 4])
    cfg["model"]["n_layer"] = trial.suggest_int("n_layer", 1, 3)

    # Batch size (GPU memory consideration)
    cfg["train"]["batch_size"] = trial.suggest_categorical("batch_size", [16, 32, 64])
