...
class StridedConvolutionalEncoder(nn.Module):
    ...
    def __init__(self, channels, kernel_size, num_layers, adapter_dim=None):
        ...
        self.gnn_layers = nn.ModuleList([GCNConv(channels, channels) for _ in range(4)])  # Add GNN layers
        
        if adapter_dim is not None:
            self.adapter_layers = nn.ModuleList([nn.Sequential(
                nn.Linear(channels, adapter_dim),
                nn.ReLU(),
                nn.Linear(adapter_dim, channels)
            ) for _ in range(2)])
        else:
...
class BaselineModel(nn.Module):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, kernel_size, recon_type="mse", content_cosine=False, time_resolution=256, losses=['recon'], adapter_dim=None):
        ...
        self.encoder = StridedConvolutionalEncoder(channels, kernel_size, num_layers, adapter_dim)
        ...
        self.decoder = TransposedConvolutionalDecoder(channels, kernel_size, num_layers, adapter_dim)
...
class SplitLatentModel(BaselineModel):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, kernel_size, adapter_dim=None, recon_type=True, content_cosine=False):
        super(SplitLatentModel, self).__init__(in_channels, channels, latent_dim, num_layers, kernel_size, recon_type, content_cosine, adapter_dim=adapter_dim)
