...
class CustomLoader():
    ...
    def __init__(self, data_dict, split='train'):
        ...
        # Default ERP-Core subject splits. For generic datasets, we support split='all'
        ...
        if split == 'dev':
            ...
            # Generic path: include all subjects present in the data_dict
            self.unique_subjects = sorted(list(set(self.subjects.tolist())))
        else:
            raise ValueError('Invalid split')

        # Task/run metadata. For generic datasets, provide minimal placeholders
        self.task_to_label = data_dict.get('labels', {0: 'Generic'})
        if not isinstance(self.task_to_label, dict):
            # Some datasets may store labels as a tensor; convert to mapping
            uniques = np.unique(self.tasks)
            self.task_to_label = {int(t): str(int(t)) for t in uniques}
        self.run_labels = data_dict.get('run_labels', ['Generic'])
        self.unique_tasks = list(self.task_to_label.keys()) if split != 'N170' else []
        self.unique_runs = list(range(1, len(self.run_labels) + 1))
        if split == 'N170':
            self.unique_tasks = [t for t, l in self.task_to_label.items() if str(l).startswith('N170')]
            self.unique_runs = [r + 1 for r, l in enumerate(self.run_labels) if l == 'N170']
            self.paradigms = [[8, 9]]
        else:
            # For ERP data, drop N170 by default; for generic, keep everything
            if any(str(l).startswith('N170') for l in self.task_to_label.values()):
                self.unique_tasks = [t for t, l in self.task_to_label.items() if not str(l).startswith('N170')]
                self.unique_runs = [r + 1 for r, l in enumerate(self.run_labels) if l != 'N170']
                self.paradigms = [[0, 1], [2, 3], [4, 5], [6, 7], [10, 11], [12, 13]]
            else:
                self.unique_tasks = sorted(list(set(self.tasks.tolist()))) if hasattr(self.tasks, 'tolist') else sorted(list(set(self.tasks)))
                self.paradigms = []
