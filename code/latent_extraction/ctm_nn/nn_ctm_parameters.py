...
# ------------------------------------------------------
# Helper: extract θ from a single EEG segment
# ------------------------------------------------------
def extract_theta(
    encoder: nn.Module,
    segment: mne.io.Raw,
    ...
    """Return CTM parameter estimates (θ) for one EEG segment.

    Parameters
    ----------
    encoder : Trained ``EEGToTheta`` network (weights already loaded).
    segment : EEG data of shape (19, 7680) as ``numpy`` array *or* ``torch`` tensor.
    device  : Device for forward-pass (defaults to encoder's first param device).

    Returns
    -------
    theta : (19, 8) ``torch.Tensor`` (on **CPU**) with the inferred parameters.
    """
    segment = segment.get_data()
    
    # Resolve device
    if device is None:
        try:
            device = next(encoder.parameters()).device  # type: ignore[arg-type]
        except StopIteration:
            device = torch.device("cpu")

    encoder.eval()

    # Convert input to tensor of right shape and dtype
    if not torch.is_tensor(segment):
        segment = torch.from_numpy(np.asarray(segment)).float()
    else:
        segment = segment.float()

    if segment.shape != (19, 7680):
        raise ValueError(
            f"Expected segment of shape (19, 7680), got {tuple(segment.shape)}"
        )

    segment = segment.unsqueeze(0).to(device)  # (1, 19, 7680)

    with torch.no_grad():
        theta = encoder(segment)[0].cpu()  # (19, 8)
...
def train_theta_joint(
    ...
    # Encoder
    encoder = EEGToTheta(in_len=7680, out_dim=p_dim).to(device)
    ...
    # ------------------------------
    # Frequency-band loss weighting
    # ------------------------------
    loss_w = torch.ones_like(freqs)
    band_mask = ((freqs >= 5) & (freqs <= 15)) | ((freqs >= 20) & (freqs <= 30))
    loss_w[band_mask] = 2.0  # double weight for 5-15 Hz & 20-30 Hz
    # Keep a device copy to avoid moving every iteration
    ...
    best_state_dict = None  # type: ignore[var-annotated]
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y, idx in train_dl:
            ...
            theta = encoder(x)  # (B,19,8)
            B_cur = theta.size(0)
            theta_flat = theta.view(-1, theta.size(-1))  # (B*19, 8)
            pred_psd_flat = compute_ctm_psd(theta_flat, freqs).to(device)  # (B*19, F)
            ...
            pred_psd_mean = pred_psd.mean(dim=1)       # (B,F)
            target_psd_mean = target_psd.mean(dim=1)   # (B,F)

            log_pred = torch.log10(pred_psd_mean + eps)
            log_tgt  = torch.log10(target_psd_mean + eps)

            se = (log_pred - log_tgt) ** 2             # (B,F)
            se_w = se * loss_w.view(1, -1)             # (B,F)
            loss = se_w.mean()                         # scalar
        ...
        if eval_dl is not None:
            ...
            val_running = 0.0
            with torch.no_grad():
                for x_v, _, _ in eval_dl:
                    x_v = x_v.to(device)
                    theta_v = encoder(x_v)  # (B,19,8)
                    Bv = theta_v.size(0)
                    pred_psd_v = compute_ctm_psd(theta_v.view(-1, theta_v.size(-1)), freqs).to(device)
                    pred_psd_v = pred_psd_v.view(Bv, 19, -1)
                    target_psd_v = compute_empirical_psd(x_v.cpu(), freqs).to(device)

                    pred_psd_mean_v = pred_psd_v.mean(dim=1)      # (B,F)
                    target_psd_mean_v = target_psd_v.mean(dim=1)

                    log_pred_v = torch.log10(pred_psd_mean_v + eps)
                    log_tgt_v  = torch.log10(target_psd_mean_v + eps)

                    se_v = (log_pred_v - log_tgt_v) ** 2
                    se_v_w = se_v * loss_w.view(1, -1)
                    l_val = se_v_w.mean()
                    val_running += l_val.item() * x_v.size(0)
        # -------------------------
        # Bookkeeping & logging
        # -------------------------
        train_loss = running / len(train_ds)
        if eval_ds is not None:
            val_loss = val_running / len(eval_ds)
            print(
                f"Epoch {epoch}: mean PSD loss={train_loss:.6f}, mean eval PSD loss={val_loss:.6f}"
            )
        else:
            # If no explicit validation split provided, fall back to training loss
            val_loss = train_loss
            print(f"Epoch {epoch}: mean PSD loss={train_loss:.6f}")

        # -------------------
        # Record history
        # -------------------
        train_hist.append(train_loss)
        val_hist.append(val_loss if eval_ds is not None else None)
        ...
        if val_loss < best_val_loss - 1e-8:
            ...
            if patience_counter >= patience:
                print(
                    f"Early stopping triggered: no improvement for {patience} consecutive epochs."
                )
                break
    ...
    #save example comparison plot at the end of training
    encoder.eval()
    with torch.no_grad():
        theta_s = encoder(x_sample)  # (1,19,8)
        pred_psd_s = compute_ctm_psd(theta_s.view(-1, theta_s.size(-1)), freqs).cpu().numpy().reshape(1,19,-1)
    ...
    # Convert to decibels for better visual comparison
    eps = 1e-12
    # Channel-averaged PSD comparison (matches loss) --------------
    plt.figure(figsize=(6, 4))
    plt.plot(freqs.cpu().numpy(), 10*np.log10(target_psd_avg + eps), label="Empirical", color="black")
    plt.plot(freqs.cpu().numpy(), 10*np.log10(pred_psd_avg + eps), label="CTM fit", color="red", linestyle="--")
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("PSD (dB)")
    plt.title("Channel-average PSD fit")
    plt.legend()
    out_mean = os.path.join(plot_dir if plot_dir else ".", "mean_psd_fit.png")
    plt.tight_layout()
    plt.savefig(out_mean)
    plt.close()
    print(f"Saved mean PSD fit plot to {out_mean}")
    ...
    out_dir = os.path.dirname(save_theta)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    torch.save(best_state_dict, save_theta)
    print(f"Best model saved to {save_theta} (val loss = {best_val_loss:.6f})")
    ...
    if plot_dir is not None:
        ...
        if any(v is not None for v in val_hist):
            plt.plot(epochs_range, [v if v is not None else float('nan') for v in val_hist], label="Val")
        plt.xlabel("Epoch")
        plt.ylabel("Mean PSD loss (log scale)")
        plt.title("Training progress – CTM PSD loss")
        plt.yscale("log")
        plt.legend()
        plt.tight_layout()
        out_curve = os.path.join(plot_dir, "loss_curve.png")
        plt.savefig(out_curve)
        plt.close()
        print(f"Saved loss curve to {out_curve}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_root",
        type=str,
        default="/homes/lrh24/thesis/Datasets/tuh-eeg-ab-clean",
    ...
    parser.add_argument("--beta_ce", type=float, default=1)
    parser.add_argument("--plot_dir", type=str, default=".", help="Directory to save training curves")
    parser.add_argument("--patience", type=int, default=15, help="Early-stopping patience based on validation loss")
