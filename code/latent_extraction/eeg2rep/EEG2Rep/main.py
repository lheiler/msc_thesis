...
parser.add_argument('--early_stop_patience', type=int, default=10, help='Patience for early stopping (0 disables)')
...
parser.add_argument('--warmup_epochs', type=int, default=10, help='Warmup epochs for cosine LR scheduler')
parser.add_argument('--cov_weight', type=float, default=0.04, help='Weight for covariance loss term')
...
if __name__ == '__main__':
    ...
    # In FIF mode, we bypass the legacy numpy loader and construct datasets directly in the runner
    Data = None if config.get('fif_root') else Data_Loader(config)
