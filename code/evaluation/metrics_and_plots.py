...
PAIRWISE_N_WORKERS = 8 # Conservative setting (1 = sequential)
...
def _global_compute_pair_worker(args):
    ...
    # Subsampling configuration (to prevent memory exhaustion)
    MAX_PAIRWISE_SAMPLES = 10000
    
    # Helper functions (copied from class)
    def _subsample_for_memory(Z1, ids1, Z2, ids2, max_n):
        n = Z1.shape[0]
        if n <= max_n:
            return Z1, ids1, Z2, ids2
        
        # Use a consistent seed for reproducibility
        np.random.seed(42)
        idx = np.random.choice(n, max_n, replace=False)
        return Z1[idx], [ids1[i] for i in idx], Z2[idx], [ids2[i] for i in idx]
    ...
    try:
        ...
        if i == j:
            ...
            # Add subsampling for memory stability on large datasets
            aligned_ids = list(range(Z1_aligned.shape[0])) # Dummy IDs since they are already aligned
            Z1_aligned, _, Z2_aligned, _ = _subsample_for_memory(Z1_aligned, aligned_ids, Z2_aligned, aligned_ids, MAX_PAIRWISE_SAMPLES)
...
# =====================================================================
# GEOMETRY METRICS (computed on the fly if missing from JSON)
# =====================================================================
...
class MetricsComparison:
    def __init__(self, results_dir: str = "/rds/general/user/lrh24/home/msc_thesis/code/Results", 
        ...
        Args:
            ...
            all_methods: Complete ordered list of all methods (used for canonical ordering)
        """
        self.results_dir = Path(results_dir)
        self.output_dir = self.results_dir / "metrics_and_plots"
        self.output_dir.mkdir(exist_ok=True)
        
        # Define all available methods from argument or method_group
        ...
        # Auto-detect dataset and primary task from method names
        self._detect_dataset_and_tasks()
        
        self.metrics_data = {}
        self.latent_features_cache = {}  # Cache for loaded latent features
    
    def _detect_dataset_and_tasks(self):
        """Detect the dataset prefix and relevant tasks from method names."""
        if self.methods and self.methods[0].startswith('lemon-'):
            self.dataset_prefix = 'lemon-'
            self.dataset_name = 'lemon'
            self.primary_task = 'age'
            self.all_tasks = ['age']  # omit gender
        else:
            self.dataset_prefix = 'tuh-'
            self.dataset_name = 'tuh'
            self.primary_task = 'abnormal'
            self.all_tasks = ['abnormal']  # omit gender
        
    def clean_method_name(self, method_name: str) -> str:
        """Clean method name by removing dataset prefixes."""
    ...
    def load_metrics(self) -> Dict[str, Dict]:
        """Load metrics from latent_metrics.json and final_metrics.txt for all methods."""
        print("Loading metrics from all methods...")
        
        for method in self.methods:
            method_dir = self.results_dir / method
            latent_file = method_dir / "latent_metrics.json"
            txt_file = method_dir / "final_metrics.txt"
            ...
            if latent_file.exists():
                ...
                try:
                    ...
                    # Build a combined metrics dict matching the expected structure
                    combined = {"latent": latent_data}
                    
                    # Parse final_metrics.txt into metrics_per_task structure
                    if txt_file.exists():
                        combined["metrics_per_task"] = self._parse_final_metrics_txt(txt_file)
                        combined["raw_txt"] = txt_file.read_text()
                    
                    self.metrics_data[method] = combined
                except Exception as e:
                    print(f"  ✗ Error loading {method}: {e}")
                    self.metrics_data[method] = None
            else:
                print(f"  ✗ No latent_metrics.json for {method}")
                self.metrics_data[method] = None
    ...
    def _parse_final_metrics_txt(self, txt_path: Path) -> Dict:
        """Parse final_metrics.txt into a structured dict matching the expected format."""
        import re
        metrics_per_task = {}
        txt = txt_path.read_text()
        
        # Find all tasks mentioned
        task_pattern = re.compile(r'metrics_per_task\.([a-zA-Z0-9_]+)\.([a-zA-Z0-9_]+):\s+(.+)')
        for match in task_pattern.finditer(txt):
            task_name = match.group(1)
            metric_name = match.group(2)
            value_str = match.group(3).split('#')[0].strip()
            
            if task_name not in metrics_per_task:
                metrics_per_task[task_name] = {}
            
            # Try to parse as float
            try:
                val = float(value_str)
                metrics_per_task[task_name][metric_name] = val
            except ValueError:
                metrics_per_task[task_name][metric_name] = value_str
    ...
    def extract_classification_metrics(self) -> pd.DataFrame:
        ...
        for method, data in self.available_methods.items():
            ...
            # Use dataset-aware tasks (omit gender)
            for task in self.all_tasks:
                ...
                # Also extract linear probe results
                probe_task = f"{task}_linear_probe"
                if probe_task in data['metrics_per_task']:
                    probe_metrics = data['metrics_per_task'][probe_task]
                    row = {
                        'method': method,
                        'task': probe_task,
                        'accuracy': probe_metrics.get('accuracy'),
                        'f1': probe_metrics.get('f1'),
                        'f1_macro': probe_metrics.get('f1_macro'),
                        'precision': probe_metrics.get('precision'),
                        'recall': probe_metrics.get('recall'),
                        'roc_auc': probe_metrics.get('roc_auc'),
                        'pr_auc': probe_metrics.get('pr_auc'),
                        'loss': probe_metrics.get('loss')
                    }
                    rows.append(row)
        ...
        for method, data in self.available_methods.items():
            ...
            for split in ['train', 'eval']:
                if split in data['latent']:
                    ...
                    # On-the-fly geometry metric computation if missing
                    if row['trustworthiness'] is None or row['continuity'] is None:
                        latent_features_path = self.results_dir / method / f"temp_latent_features_{split}.json"
                        if latent_features_path.exists():
                            try:
                                X_list = []
                                with open(latent_features_path, 'r') as f:
                                    for line in f:
                                        if line.strip():
                                            inner_row = json.loads(line)
                                            # In these files, inner_row[0] is the feature vector
                                            X_list.append(inner_row[0])
                                
                                X = np.array(X_list)
                                X_list = None # Free memory
                                ...
                                geometry_metrics = compute_geometry(X)
                                if 'trustworthiness' in geometry_metrics:
                                    row['trustworthiness'] = geometry_metrics['trustworthiness']
                                if 'continuity' in geometry_metrics:
                                    row['continuity'] = geometry_metrics['continuity']
                            except Exception as e:
                                print(f"  ✗ Failed to compute geometry for {method} ({split}): {e}")
    ...
    def _create_classification_summary_comparison(self, df: pd.DataFrame):
        ...
        task_acc_cols = [f'{t}_accuracy' for t in self.all_tasks if f'{t}_accuracy' in comp_df.columns]
        if len(task_acc_cols) >= 1:
            first_acc = comp_df[task_acc_cols[0]].fillna(0)
            second_acc = comp_df[task_acc_cols[1]].fillna(0) if len(task_acc_cols) > 1 else comp_df[task_acc_cols[0]].fillna(0)
            
            colors = self.get_professional_colors()
            bars1a = ax1.bar([x - width/2 for x in x_pos], first_acc, width,
                           label=f'{self.all_tasks[0].title()} Task', alpha=0.8, color=colors['primary_blue'])
            if len(task_acc_cols) > 1:
                bars1b = ax1.bar([x + width/2 for x in x_pos], second_acc, width,
                               label=f'{self.all_tasks[1].title()} Task', alpha=0.8, color=colors['primary_purple'])
        ...
        task_f1_cols = [f'{t}_f1' for t in self.all_tasks if f'{t}_f1' in comp_df.columns]
        if len(task_f1_cols) >= 1:
            first_f1 = comp_df[task_f1_cols[0]].fillna(0)
            second_f1 = comp_df[task_f1_cols[1]].fillna(0) if len(task_f1_cols) > 1 else first_f1
            
            bars2a = ax2.bar([x - width/2 for x in x_pos], first_f1, width,
                           label=f'{self.all_tasks[0].title()} Task', alpha=0.8, color=colors['primary_blue'])
            if len(task_f1_cols) > 1:
                bars2b = ax2.bar([x + width/2 for x in x_pos], second_f1, width,
                               label=f'{self.all_tasks[1].title()} Task', alpha=0.8, color=colors['primary_purple'])
        ...
        task_auc_cols = [f'{t}_roc_auc' for t in self.all_tasks if f'{t}_roc_auc' in comp_df.columns]
        if len(task_auc_cols) >= 1:
            first_auc = comp_df[task_auc_cols[0]].fillna(0)
            second_auc = comp_df[task_auc_cols[1]].fillna(0) if len(task_auc_cols) > 1 else first_auc
            
            bars3a = ax3.bar([x - width/2 for x in x_pos], first_auc, width,
                           label=f'{self.all_tasks[0].title()} Task', alpha=0.8, color=colors['primary_blue'])
            if len(task_auc_cols) > 1:
                bars3b = ax3.bar([x + width/2 for x in x_pos], second_auc, width,
                               label=f'{self.all_tasks[1].title()} Task', alpha=0.8, color=colors['primary_purple'])
        ...
        has_some_metrics = any(f'{t}_accuracy' in comp_df.columns for t in self.all_tasks)
        if has_some_metrics:
    ...
    def _create_metric_plot(self, data: pd.DataFrame, metric: str, task: str):
        ...
        if metric in ['accuracy', 'f1']:
            ...
            for i, (bar, val) in enumerate(zip(bars, data[metric])):
                ax.text(float(val) + 0.005, bar.get_y() + bar.get_height()/2, 
    ...
    def _create_efficiency_performance_plot(self, data: pd.DataFrame):
        ...
        if efficiency_data:
            ...
            # Plot 1 & 2: Efficiency vs Task MI (dynamically from self.all_tasks)
            task_axes = [ax1, ax2]
            task_colors = ['#5B9BD5', '#FF7F50']
            for t_idx, task in enumerate(self.all_tasks[:2]):
                ax = task_axes[t_idx]
                mi_col = f'mi_{task}'
                if mi_col in eff_df.columns:
                    task_data = eff_df.dropna(subset=['efficiency', mi_col])
                    if not task_data.empty:
                        ax.scatter(task_data['efficiency'], task_data[mi_col],
                                  s=task_data['active_units']*10, alpha=0.7,
                                  c=task_colors[t_idx], edgecolors='black')
                        
                        for i, (eff, mi, method) in enumerate(zip(task_data['efficiency'], 
                                                                task_data[mi_col],
                                                                task_data['method'])):
                            ax.annotate(method, (eff, mi), xytext=(5, 5),
                                       textcoords='offset points', fontsize=8)
                        
                        ax.set_xlabel('Dimensional Efficiency')
                        ax.set_ylabel(f'{task.title()} MI')
                        ax.set_title(f'Efficiency vs {task.title()} Informativeness\n(Bubble size = Active Units)')
                        ax.grid(True, alpha=0.3)
    ...
    def _create_information_summary_comparison(self, data: pd.DataFrame):
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'Information Content: Method Comparison ({self.dataset_name.upper()})', fontsize=18, fontweight='bold')
        
        plot_axes = [ax1, ax2]
        plot_colors_list = ['#5B9BD5', '#FF7F50']
        
        # Plot 1 & 2: Task-specific MI (dynamically)
        for t_idx, task in enumerate(self.all_tasks[:2]):
            ax = plot_axes[t_idx]
            mi_col = f'mi_{task}'
            if mi_col in info_df.columns:
                task_data = info_df.dropna(subset=[mi_col])
                if not task_data.empty:
                    bars = ax.bar(range(len(task_data)), task_data[mi_col], 
                                alpha=0.8, color=plot_colors_list[t_idx])
                    ax.set_xticks(range(len(task_data)))
                    self.set_colored_xticklabels(ax, task_data['method'], rotation=45, ha='right')
                    ax.set_ylabel('Mutual Information')
                    ax.set_title(f'{task.title()} Task Informativeness')
                    ax.grid(True, alpha=0.3, axis='y')
                    for i, (bar, val) in enumerate(zip(bars, task_data[mi_col])):
                        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + val*0.01,
                               f'{val:.3f}', ha='center', va='bottom', fontweight='bold')
        
        # Plot 3: Average MI across available tasks
        mi_task_cols = [f'mi_{t}' for t in self.all_tasks if f'mi_{t}' in info_df.columns]
        if mi_task_cols:
            avg_data = info_df.dropna(subset=mi_task_cols)
            if not avg_data.empty:
                avg_mi = avg_data[mi_task_cols].mean(axis=1)
                colors = self.get_professional_colors()
                bars3 = ax3.bar(range(len(avg_data)), avg_mi, alpha=0.8, color=colors['primary_blue'])
                ax3.set_xticks(range(len(avg_data)))
                self.set_colored_xticklabels(ax3, avg_data['method'], rotation=45, ha='right')
        ...
        # Plot 4: Side-by-side comparison (if multiple tasks)
        if len(mi_task_cols) >= 2:
            comparison_data = info_df.dropna(subset=mi_task_cols)
            if not comparison_data.empty:
                colors = self.get_professional_colors()
                x_pos = np.arange(len(comparison_data))
                width = 0.8 / len(mi_task_cols)
                
                for i, (mi_col, task) in enumerate(zip(mi_task_cols, self.all_tasks)):
                    offset = (i - len(mi_task_cols)/2 + 0.5) * width
                    ax4.bar(x_pos + offset, comparison_data[mi_col], width,
                           label=f'{task.title()} Task', alpha=0.8, 
                           color=plot_colors_list[i % len(plot_colors_list)])
                ...
                ax4.legend()
                ax4.grid(True, alpha=0.3, axis='y')
        elif len(mi_task_cols) == 1:
            # Only one task — just show a ranked view
            task = self.all_tasks[0]
            mi_col = mi_task_cols[0]
            ranked = info_df.dropna(subset=[mi_col]).sort_values(mi_col, ascending=True)
            if not ranked.empty:
                colors_ranked = plt.cm.RdYlGn(np.linspace(0.3, 1.0, len(ranked)))
                bars4 = ax4.barh(range(len(ranked)), ranked[mi_col], color=colors_ranked, alpha=0.8)
                ax4.set_yticks(range(len(ranked)))
                self.set_colored_yticklabels(ax4, ranked['method'])
                ax4.set_xlabel('Mutual Information')
                ax4.set_title(f'{task.title()} MI Ranking')
    ...
    def _compute_hierarchical_method_order_for_matrix(self, matrix: pd.DataFrame):
        ...
        try:
            ...
            # Ensure matrix is clean (no NaNs or Infs) for clustering
            clean_matrix = matrix.fillna(0.0).replace([np.inf, -np.inf], 0.0)
            
            # Use seaborn's clustermap to get optimal ordering
            # This automatically handles the clustering and reordering
            temp_clustermap = sns.clustermap(clean_matrix, method='ward', metric='euclidean',
    ...
    def _create_mi_comparison_plot(self, data: pd.DataFrame):
        """Create MI comparison for the dataset's relevant tasks."""
        mi_data = []
        for _, row in data.iterrows():
            row_data = {'method': row['method_clean']}
            has_any = False
            for task in self.all_tasks:
                mi_col = f'mi_{task}_mean'
                if mi_col in row and not pd.isna(row.get(mi_col)):
                    row_data[task] = row[mi_col]
                    has_any = True
            if has_any:
        ...
        if mi_data:
            ...
            colors_list = ['#5B9BD5', '#87CEEB', '#FF7F50']
            x_pos = np.arange(len(mi_df))
            width = 0.8 / max(len(self.all_tasks), 1)
            
            for i, task in enumerate(self.all_tasks):
                if task in mi_df.columns:
                    offset = (i - len(self.all_tasks)/2 + 0.5) * width
                    ax.bar(x_pos + offset, mi_df[task], width,
                          label=f'{task.title()} Task', alpha=0.8,
            ...
            ax.set_title(f'Feature Informativeness ({self.dataset_name.upper()})', 
                        fontsize=16, fontweight='bold')
            ax.legend()
            ax.grid(True, alpha=0.3, axis='y')
            plt.tight_layout()
    ...
    def create_summary_tables(self, class_df: pd.DataFrame, latent_df: pd.DataFrame, pca_df: pd.DataFrame):
        ...
        # Enhanced metrics (dynamically include task-specific MI metrics)
        enhanced_metrics = [
            'variance_entropy', 'effective_dim_variance', 'variance_concentration',
        ]
        for task in self.all_tasks:
            enhanced_metrics.append(f'mi_{task}_mean')
            enhanced_metrics.append(f'mi_{task}_concentration')
        ...
        for method in methods_list:
            ...
            if not latent_data.empty:
                ...
                # Enhanced metrics
                enhanced_latent_metrics = ['variance_entropy'] + [f'mi_{t}_mean' for t in self.all_tasks]
                ...
                na_metrics = ['dim', 'active_units', 'hsic_global_score', 'silhouette', 'davies_bouldin', 'calinski_harabasz', 'trustworthiness', 'continuity', 'dist_corr',
                              'variance_entropy', 'efficiency'] + [f'mi_{t}_mean' for t in self.all_tasks]
...
def main():
    ...
    for ds_prefix in ["tuh", "lemon"]:
        print(f"\n{'#'*60}")
        print(f"# DATASET: {ds_prefix.upper()}")
        print(f"{'#'*60}")
        
        small_aggregated = [f"{ds_prefix}-{m}" for m in [
            "ctm_cma_avg", "ctm_nn_avg", "hopf_avg", "jr_avg",
            "wong_wang_avg", "pca_avg", "psd_ae_avg"
        ]]
        medium_unrestricted = [f"{ds_prefix}-{m}" for m in [
            "ctm_nn_pc", "hopf_pc", "jr_pc", "c22",
            "pca_pc", "psd_ae_pc", "eegnet"
        ]]
        
        all_methods = small_aggregated + medium_unrestricted
        method_groups = [
            {f"{ds_prefix}_small_aggregated": small_aggregated},
            {f"{ds_prefix}_medium_unrestricted": medium_unrestricted},
        ]
        ...
        for method_group in method_groups:
            ...
            # Delete the output directory for the current group before running
